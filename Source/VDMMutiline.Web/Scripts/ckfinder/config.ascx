...
<script runat="server">
	...
	{
        /*
        MembershipUser curMember = Membership.GetUser();
        if (curMember == null || Session["CurrentUser"]==null)
        {
            Response.Redirect("/Login.aspx", true);
        }
        else
        {
            user = new Moore.CMSV2.UserHelper(curMember.UserName.ToLower().Trim());
        }
        */
		...
		// The base URL used to reach files in CKFinder through the browser.
         var CurrentUserobj=  (VDMMutiline.Core.Models.VDMUser)Session["CurrentUser"] ;
            if(CurrentUserobj!=null)
            {
               BaseUrl = "~/Upload/Uploads/"+ CurrentUserobj.UrlDomain1.Replace(".","_")+"/";
            }
        ...
        type.Url = string.Format("{0}files/", BaseUrl);
        type.Dir = BaseDir == "" ? "" : string.Format("{0}files/{1}/", BaseDir, Year);

        string filePath = string.Format("{0}files/{1}/{2}/", BaseUrl, Year, Month);
        ...
        type.AllowedExtensions = new string[] { "bmp", "doc", "docx", "flv", "gif", "jpeg", "jpg", "mov", "mp3", "mp4", "mpc", "mpeg", "mpg", "pdf", "png", "ppt", "pptx", "rar", "swf", "txt", "wav", "wma", "wmv", "xls", "xlsx", "zip"};
		type.DeniedExtensions = new string[] { };

        //////////////////////////////// Images -------------------------------------------------        
		type = ResourceType.Add( "Images" );
        //type.Url = string.Format("{0}images/{1}/{2}/{3}/", BaseUrl, user.UserID, Year, Month);
        //type.Dir = BaseDir == "" ? "" : string.Format("{0}images/{1}/{2}/{3}/", BaseDir, user.UserID, Year, Month);
                
        type.Url = string.Format("{0}images/", BaseUrl);
        type.Dir = BaseDir == "" ? "" : string.Format("{0}images/{1}/", BaseDir, Year);

        string imgPath = string.Format("{0}images/{1}/{2}/{3}/", BaseUrl, Year, Month, Day);
        // Determine whether the directory exists. 
        ...
        type.Url = string.Format("{0}videos/", BaseUrl);
        type.Dir = BaseDir == "" ? "" : string.Format("{0}videos/{1}/{2}/", BaseDir, Year, Month);
        
        string videoPath = string.Format("{0}videos/{1}/{2}/", BaseUrl, Year, Month);
        // Determine whether the directory exists. 
        if (!System.IO.Directory.Exists(Server.MapPath(videoPath)))
        {
            System.IO.Directory.CreateDirectory(Server.MapPath(videoPath));
        }
        ...
        type.Url = string.Format("{0}flashs/", BaseUrl);
        type.Dir = BaseDir == "" ? "" : string.Format("{0}flashs/{1}/{2}/", BaseDir, Year, Month);

        string flashPath = string.Format("{0}flashs/{1}/{2}/", BaseUrl, Year, Month);
        // Determine whether the directory exists. 
        if (!System.IO.Directory.Exists(Server.MapPath(flashPath)))
        {
            System.IO.Directory.CreateDirectory(Server.MapPath(flashPath));
        }
        ...
        //////////////////////////////// FileStatics -------------------------------------------------
        type = ResourceType.Add("FileStatics");
        type.Url = string.Format("{0}filestatics/", BaseUrl);
        type.Dir = BaseDir == "" ? "" : string.Format("{0}filestatics/", BaseDir);

        type.MaxSize = 0;
        type.AllowedExtensions = new string[] { "html", "htm", "doc", "docx", "pdf", "txt", "bmp", "gif", "jpeg", "jpg", "png", "swf" };
        type.DeniedExtensions = new string[] { };
	}
</script>
