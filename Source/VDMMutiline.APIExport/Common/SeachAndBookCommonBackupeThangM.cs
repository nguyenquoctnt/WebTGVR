...
namespace VDMMutiline.SeachAndBook
{
    public class SeachAndBookCommon : BaseSeach
    {
         ...
         int? Adult, int? Children, int? Infant, string AirlineCode, string Currency, string older, string typeolder)
        {
            ...
            var JQDATA = SeachByAirline(request, Itinerary, DepartureAirportCode, DestinationAirportCode, startdate, enddate, "JQ");
            ...
            if (VNData != null)
            {
                if (VNData.Result != null)
                {
                    ...
                    if (data.ListFareData != null)
                    {
                        var listfare = data.ListFareData.ToList();
                        listfare.AddRange(VNData.Result.ListFareData.ToList());
                        data.ListFareData = listfare.ToArray();
                    }
                    var lstvalue = await AddLccFlightInfoAsync(VNData.Result.ListFareData, Itinerary ?? 0, listsetting, Adult ?? 0, Children ?? 0,
                            Infant ?? 0, VNData.Result.Session, listhanche);
            ...
            if (JQDATA != null)
            {
                if (JQDATA.Result != null)
                {
                    ...
                    {
                        var listfare = data.ListFareData.ToList();
                        listfare.AddRange(JQDATA.Result.ListFareData.ToList());
                        data.ListFareData = listfare.ToArray();
                    }
                    var lstvalue = await AddLccFlightInfoAsync(JQDATA.Result.ListFareData, Itinerary ?? 0, listsetting, Adult ?? 0, Children ?? 0,
            ...
            if (VJDATA != null)
            {
                if (VJDATA.Result != null)
                {
                    ...
                    {
                        var listfare = data.ListFareData.ToList();
                        listfare.AddRange(VJDATA.Result.ListFareData.ToList());
                        data.ListFareData = listfare.ToArray();
                    }
                    var lstvalue = await AddLccFlightInfoAsync(VJDATA.Result.ListFareData, Itinerary ?? 0, listsetting, Adult ?? 0, Children ?? 0,
                            Infant ?? 0, VJDATA.Result.Session, listhanche);
                    lstFlightConvert.AddRange(lstvalue);
                }
            }
            if (QHDATA != null)
            {
                if (QHDATA.Result != null)
                {
                    ...
                    {
                        var listfare = data.ListFareData.ToList();
                        listfare.AddRange(QHDATA.Result.ListFareData.ToList());
                        data.ListFareData = listfare.ToArray();
                    }
                    var lstvalue = await AddLccFlightInfoAsync(QHDATA.Result.ListFareData, Itinerary ?? 0, listsetting, Adult ?? 0, Children ?? 0,
                            Infant ?? 0, QHDATA.Result.Session, listhanche);
                    lstFlightConvert.AddRange(lstvalue);
                }
            }
        ...
        public int SaveBooking(List<SettingUserInfo> listsetting, BK_Booking item, SeachParam param, string codegiamgia, string code, VDMUser userInfo)
        {
            ...
            item.SessionID = param.DepartureFlight.SessionId;
            item.Status = 1;
        ...
        public int SaveBookingInternational(List<SettingUserInfo> listsetting, BK_Booking item, SeachParam param, string codegiamgia, string code, VDMUser userInfo)
        {
            ...
            if (param.FareDataInfo.ListFlight.Count() > 0)
            {
                ...
                if (input != null && inputdetail != null)
                {
                    ...
                    var dao = new BK_BookingDao();
                    item.SessionID = param.FareDataInfo.SessionId;
                    item.Status = 1;
                    item.Ways = param.Passengerlist.Count;
        ...
        public int SavePassenger(BK_PassengerInfo item, SeachParam param)
        {
            ...
            if (param.DepartureFlight != null)
            {
                ...
                else if (param.DepartureFlight.Airline == "VN")
                {
                    ...
                    dbItem.BaggageDepartValue = (item.TypeID == Constant.TypePassenger.EmBe ? "10" : "20");
                }
                else
                {
                    dbItem.BaggageDepartValue = item.BaggageDepartValue;
            ...
            if (param.ReturnFlight != null)
            {
                ...
                {
                    dbItem.BaggageReturnValue = (item.TypeID == Constant.TypePassenger.EmBe ? "10" : "20");
                }
                else
                {
                    dbItem.BaggageReturnValue = item.BaggageReturnValue;
