...
namespace VDMMutiline.Templeate1.MB.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public ActionResult ProfileMember(VDMUser model, FormCollection collection)
        {
            ...
            var iconlink = UploadFile.UpLoadFile(Request.Files["ConfirmedFile"], "Upload/Profile/");
            if (!string.IsNullOrEmpty(iconlink))
            {
                user.PassportPic1 = iconlink;
            }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            else
            {
                ...
                {
                    if (objcheck.ParentId.HasValue)
                    {
                        var objparent = CommonUI.GetInforByDomain(VDMMutiline.Core.Common.SiteMuti.Getsitename());
                        if (objparent != null)
                        {
                            if (objcheck.ParentId != objparent.Id)
                            {
                            ...
                            case SignInStatus.Success:
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        var getvalue = listidquyen.Where(z => z == 1);
                                        if (getvalue.Count() > 0)
                                            return RedirectToLocal(returnUrl);
                                        else
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (!UserNameExists(model.UserName))
                    {
                        ...
                        if (objparent == null)
                        {
                            ViewBag.Erro = "Không tìm thấy đại lý.";
                            return View(model);
                        }
                        var user = new VDMUser
                        {
                            ...
                            LockoutEnabled = true,
                            ParentId = objparent != null ? objparent.Id : 0,
                            Isdelete = false,
                            SiteId = 1,
                        };
