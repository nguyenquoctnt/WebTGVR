...
namespace VDMMutiline.WebBackEnd.Areas.QuanLyVe.Controllers
{
    public class BookingController : BaseController
    {
        ...
        public ActionResult Index(int? page, string thoigiandatcho, string booker, int? trangthaive, string hangvanchuyen, string seach)
        {
          ...
          //  biz.SearchCount(paramcount);
           // param.TicketCountInfos = paramcount.TicketCountInfos;
            ...
            ViewBag.showchonuser = ((IsAdmin() || GetlistRole().Contains(23)) && CurrentUser.ParentId.HasValue == false);
        ...
        public ActionResult CapNhapghichu(BK_BookingParam param)
        {
            ...
            return RedirectToAction("Create", "Booking", new { id = param.Id });
        }
        public ActionResult Inve(int bookingid)
        {
            ...
            if (inforbk != null)
            {
                ...
                //filename += ".pdf";
                //var report = new VDMMutiline.SendMailAndSMS.TeampleateVe.InVe(bookingid, GetSettingByUser(inforbk.UserId, true));
        ...
        public ActionResult Yeucauxuathoadon(BK_BookingParam param)
        {
            ...
            return RedirectToAction("Create", "Booking", new { id = param.bookingid });
        }
        ...
        public ActionResult Updatehinhthucthanhtoan(tbl_PaymentInfo modelinput)
        {
            ...
            return RedirectToAction("Create", "Booking", new { id = modelinput.IdBooking });
        }
        public ActionResult sendsms(int bookingid)
        {
            ...
            if (inforbk != null)
            {
                ...
                // ViewBag.userInfo = GetInforByDomain(inforbk.UserCreate, true);
        ...
        public ActionResult sendsms(BK_BookingInfo BookingInfo)
        {
            ...
            if (inforbk != null)
            {
                ...
                return RedirectToAction("Create", "Booking", new { id = inforbk.ID });
            }
            return View();
        }
        [HttpGet]
        ...
        public ActionResult sendmail(MailInfo Mailinfo)
        {
            ...
            if (inforbk != null)
            {
                ...
                var userInfo = GetInforByUserOrUserParent(inforbk.UserCreate);
                var html = RenderTeamPletave.Teampleateve(inforbk, listseting, Constant.TemPleateHTMLID.VeXacNhanHanhTrinh, userInfo.Id);
            ...
            return RedirectToAction("Create", "Booking", new { id = Mailinfo.Bookingid });
        }
                        ...
                        objcheck.Sendmailgannhat < DateTime.Now.AddMinutes(-1))
                    {
                        ...
                        if (obj.Status == Constant.StatusVe.DaXuatVe)
                        {
                            ...
                            if (inforbk != null)
                            {
                                ...
                                var userInfo = GetInforByUserOrUserParent(inforbk.UserCreate);
                                var html = RenderTeamPletave.Teampleateve(inforbk, listseting, Constant.TemPleateHTMLID.KichHoatVe, userInfo.Id);
                                string filename = "";
                                filename = inforbk.CreatedDate.HasValue ? inforbk.CreatedDate.Value.ToString("dd/MM/yyyy").Replace('/', '_') : "";
                                filename += "_" + inforbk.BookCode;
                                filename += "_" + Utility.ConvertToUnsign(inforbk.Name).Replace(' ', '_');
...
", code != null ? code.AttemptedValue : "",
     ...
     nguoiyeucau);
                    ...
                    return RedirectToAction("Create", "Booking", new { id = bookingidvl.HasValue ? bookingidvl.Value : 0 });
                }
            }
            catch (Exception)
            {
        ...
        public ActionResult Capnhapgiobay(FormCollection input)
        {
            ...
            return RedirectToAction("Create", "Booking", new { id = bookingidvl });
        }
        public ActionResult deletevechitiet(int bookingid, int ticketid)
        {
            ...
            return RedirectToAction("Create", "Booking", new { id = bookingid });
        }
                ...
                bokingdao.updateTen(ticketidvl.Value, (Ho != null ? Utility.ConvertToUnsign(Ho.AttemptedValue).ToUpper() : ""),
                    ...
                    hkBaggageReturnValue != null ? hkBaggageReturnValue.AttemptedValue.Replace("Kg", "").Replace("kg", "") : "",
                      Utility.ConvertToDecimal(hkBaggageReturnPrice != null ? hkBaggageReturnPrice.AttemptedValue.Replace(",", "").Replace(".", "").Trim() : ""),
                       hkgioitinh != null ? hkgioitinh.AttemptedValue : "", bookingidvl
                , birthday);
