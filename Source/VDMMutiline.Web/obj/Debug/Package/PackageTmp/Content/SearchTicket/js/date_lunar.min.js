...
function GetDateStringMin(dd, mm, yyyy) {
    var lunarDate = getLunarDate(dd, mm, yyyy); var s; var dayOfWeek = TUAN[(lunarDate.jd + 1) % 7]; s = dayOfWeek + ' ' + (lunarDate.day < 10 ? "0" : "") + lunarDate.day + "/" + (lunarDate.month < 10 ? "0" : "") + lunarDate.month; if (lunarDate.leap == 1) { s = s + "N" }
    return s
}
...
function getGioHoangDao(jd) {
    var chiOfDay = (jd + 1) % 12; var gioHD = GIO_HD[chiOfDay % 6]; var ret = ""; var count = 0; for (var i = 0; i < 12; i++) { if (gioHD.charAt(i) == '1') { ret += CHI[i]; if (count++ < 5) ret += ', ' } }
    return ret
}
...
function printTable(mm, yy) {
    var i, j, k, solar, lunar, cellClass, solarClass, lunarClass; var currentMonth = getMonth(mm, yy); if (currentMonth.length == 0) return; var ld1 = currentMonth[0]; var emptyCells = (ld1.jd + 1) % 7; var MonthHead = mm + "/" + yy; var LunarHead = getYearCanChi(ld1.year); var res = ""; res += ('<table class="thang" border="0" bgcolor="#cccccc" cellpadding="2" cellspacing="1" width="' + PRINT_OPTS.tableWidth + '">\n'); res += printHead(mm, yy); for (i = 0; i < 6; i++) {
...
function printHead(mm, yy) {
    var res = ""; var monthName = mm + "/" + yy; res += ('<tr><td colspan="2" class="navi-l">' + getPrevMonthLink(mm, yy) + '</td>\n'); res += ('<td colspan="3" class="tenthang" onClick="showMonthSelect();">' + monthName + '</td>\n'); res += ('<td colspan="2" class="navi-r">' + getNextMonthLink(mm, yy) + '</td></tr>\n'); res += ('<tr onClick="alertAbout();">\n'); for (var i = 0; i <= 6; i++) { res += ('<td class=ngaytuan>' + DAYNAMES[i] + '</td>\n') }
    res += ('<\/tr>\n'); return res
}
function printEmptyCell() { return '<td class=ngaythang><div class=cn>&nbsp;</div> <div class=am>&nbsp;</div></td>\n' }
function printCell(lunarDate, solarDate, solarMonth, solarYear) {
    var cellClass, solarClass, lunarClass, solarColor; cellClass = "ngaythang"; solarClass = "t2t6"; lunarClass = "am"; solarColor = "black"; var dow = (lunarDate.jd + 1) % 7; if (dow == 0) { solarClass = "cn"; solarColor = "red" } else if (dow == 6) { solarClass = "t7"; solarColor = "green" }
