...
{
    public class TblArticleCateController : BaseController
    {
        ...
        public ActionResult Index(int? TypeID)
        {
            ...
            var reparelist = (param.SysCategoryInfos.Where(z=>z.IsContend!=true).Select(n => new SelectListItem() { Text = n.TenHienthi, Value = n.Id.ToString() })).ToList();
            ViewBag.DataTree = reparelist;
            return View(TypeID);
        }
        public ActionResult AjaxLoadList(GridFilterSetting<TblArticleInfo> gridFilterSetting, string keyseach, int? Types, int? parentcateFillter)
        {
            ...
            var tblArticleFilter = new TblArticleFilter() { Search = keyseach, typeid = Types,
                ListUserName = GetparentUserDaiLy().Select(z => z.UserName).ToList() ,
            ChuyenMucID= parentcateFillter
            };
        ...
        public ActionResult Create(int? id, int? typeid)
        {
            ...
            var reparelist = param.SysCategoryInfos.Where(z => z.IsContend != true).ToList();
